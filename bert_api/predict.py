...
from vectorizers import bert_vectorizer
...
class JointBertModel1(NLUModel):
    ...
    def load(load_folder_path):
        ...
        new_model = JointBertModel(intents_num, bert_hub_path, num_bert_fine_tune_layers, is_bert)
...
def predict(utterance):
    ...
    print(utterance)
    input_ids, input_mask, segment_ids, data_sequence_lengths = bert_vectorizer.transform([utterance])
    predicted_intents = model.predict([input_ids, input_mask, segment_ids])
    print(dict1[str(np.argmax(predicted_intents))])
    print(np.amax(predicted_intents))
